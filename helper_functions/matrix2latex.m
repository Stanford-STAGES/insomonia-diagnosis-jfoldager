...
% Enjoy life!!!
    ...
    okargs = {'rowlabels','columnlabels', 'alignment', 'format', 'size','longtable','lines'};
    for j=1:2:(nargin-2)
        pname = varargin{j};
        pval = varargin{j+1};
    ...
    if(longtable)
        if lines,fprintf(fid, '\\begin{longtable}{|');else,fprintf(fid, '\\begin{longtable}{');end
    else
        
        if lines,fprintf(fid, '\\begin{tabular}{|');else,fprintf(fid, '\\begin{tabular}{');end
    end
    

    if(~isempty(rowLabels))
        if lines,fprintf(fid, 'l|');else,fprintf(fid, 'l|');end
    end
    for i=1:width
        if lines,fprintf(fid, '%c|', alignment);else,fprintf(fid, '%c', alignment);end
    end
    fprintf(fid, '}\r\n');
