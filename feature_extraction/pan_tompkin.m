function [qrs_amp_raw,qrs_i_raw,ECG,delay]=pan_tompkin(ecg,fs,gr,epochLength)
...
[pks,locs] = findpeaks(ecg_m,'MinPeakDistance',round(0.2*fs));
    ...
    ECG = reshape(ecg_m(1:end-mod(length(ecg_m) ...
                ,fs*epochLength)),fs*epochLength,[]);
%     pkss = zeros(length(ecg_m),1);
%     lkss = zeros(length(ecg_m),1);
%     idx  = 1;
%     for i = 1:size(ECG,2) % Windowing
%         [pks,lk] = findpeaks(ECG(:,i),'MinPeakDistance',round(0.2*fs));
%         pkss(idx:idx+length(lk)-1) = pks;
%         lkss(idx:idx+length(lk)-1) = lk+(i-1)*fs*30;
%         idx = idx + length(lk);
% %         fprintf('%3f\n',i/size(ECG,2));
%     end
% pks     = pkss(lkss > 0);
